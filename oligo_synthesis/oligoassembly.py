# Module supplying methods/classes for generating overlapping oligo sequences from a gene sequence.
...
class OligoAssembly(object):
    ...
    def __init__(self, seq, tm=65, oligo_size=120, require_even=True, start_5 = True):
       
        results = oligo_calc(seq,tm,oligo_size,require_even,start_5)

        self.oligos = results['oligos']
        self.overlaps = results['overlaps']
        self.overlap_tms = results['overlap_tms']
    ...
    def write(self,outpath):
        oligo_writer = csv.writer(open(outpath,'wb'),delimiter=',',quoting=csv.QUOTE_NONNUMERIC)
        oligo_writer.writerow(['oligo','overlap','overlap_tm'])
        for i in range(len(self.oligos)-1):
            oligo_writer.writerow([self.oligos[i], self.overlaps[i],self.overlap_tms[i]])
...
def oligo_calc(seq, tm=65, oligo_size=120, require_even=False, start_5 = True):
    ...
    oligo_n = int(ceil(float(len(seq))/oligo_size)) # Absolute minimum n oligos
    if require_even:
        oligo_increment = 2
        if oligo_n%2==1:
            oligo_n += 1
    ...
    # Loop until all overlaps meet minimum Tm (overlaps extended to full oligo_size)
    while( any( [ x <= tm for x in overlap_tms ] ) ):
        
        overlap_tms = [ min_tm ] * (oligo_n-1)
        print('Trying with ' + str(oligo_n) + ' oligos.')
        # initial overlap locations
        o_l = [int((x+1)*float(len(seq))/(oligo_n)) for x in range(oligo_n-1)]
        # overlap starts
        o_s = [0] + o_l
        # overlap ends
        ...
        o_nonmaxed = range(oligo_n-1)
        while not all(maxed):
            # Find lowest overlap Tm index location
            # If more than one entry, it just picks the earliest one
            # Ignores any that are maxed
            nonmaxed_tms = [i for j,i in enumerate(overlap_tms) if j in o_nonmaxed] 
            min_tm = overlap_tms.index(min(nonmaxed_tms))
            ...
            if oligo_l==oligo_size&oligo_r==oligo_size:
                ...
                o_nonmaxed.pop(o_nonmaxed.index(min_tm))
