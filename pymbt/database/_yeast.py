...
def fetch_yeast_locus_sequence(locus_name, flanking_size=0):
    ...
    if flanking_size > 0:
        ...
        query.add_view(
                       ...
                       "sgdAlias","organism.shortName", "sequence.length", 
                       "sequence.residues", "description","qualifier"
                       )
        ...
        query.add_constraint("Gene", "LOOKUP", locus_name, "S. cerevisiae", code = "E")
...
def get_gene_id(gene_name):
    ...
    query.add_view(
                   ...
                   "crossReferences.identifier", "crossReferences.source.name"
                   )
    ...
    for row in query.rows():
        id= row["secondaryIdentifier"]
    return id
    
def get_yeast_promoter_ypa(gene_name):
    loc=get_yeast_gene_location(gene_name)
    id=get_gene_id(gene_name)
    ypa_url="http://ypa.csbb.ntu.edu.tw/do?act=download&nucle=InVitro&right="+str(loc[2])+"&left="+str(loc[1])+"&gene="+str(id)+"&chr="+str(loc[0])
