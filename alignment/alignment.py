...
class Sanger:
    ...
    def __init__(self,ref,res):
        ...
        self.needle = [(self.alignments[i],self.scores[i]) for i,x in enumerate(self.alignments) ]
        ...
        self.ranges = [ (x.find('X') , (len(x)-x[::-1].find('X'))) for x in self.gaps ]
            ...
            for j,y in enumerate(x):
                if a_refs[i][j] is not '-' and a_res[i][j] is not '-':
                    if a_refs[i][j] is not a_res[i][j]:
                        if self.mismatches.has_key(curkey):
...
def sequences_display(sequences,start_stop,context=10,indent=10):
    if len(sequences) != 2:
    ...
    seq_list_1 = []
    seq_list_2 = []
    seq_list_1.append(sequences[0][max(start-context,0):start])
    seq_list_2.append(sequences[1][max(start-context,0):start])
    seq_list_1.append(sequences[0][start:stop+1])
    seq_list_2.append(sequences[1][start:stop+1])
    seq_list_1.append(sequences[0][stop+1:min(stop+context,len(sequences[0]))])
    seq_list_2.append(sequences[1][stop+1:min(stop+context,len(sequences[0]))])
